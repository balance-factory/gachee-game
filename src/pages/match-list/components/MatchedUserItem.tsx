...
const MatchedUserItem: React.FC<MatchedUserItemProps> = ({
  matchedUserInfo,
}) => {
  ...
  const handleClickMatchResult = (user: Interface.MatchedUser) => {
    localStorage.setItem(
      "matchUserInfo",
      JSON.stringify({ name: user.name, userScore: user.matchScore })
    );
  ...
  return (
    ...
    >
      {matchedUserInfo.profileImage ? (
        <UserImg src={matchedUserInfo.profileImage} />
      ) : (
        <UserEmptyImg />
      )}
